# Phase 02 - Contributor Experience And CI
...
**Goal:** Complete the Contributor Experience And CI slice while preserving prior phase contracts and downstream handoff boundaries.
...
```json
{
  ...
  "phase": {
    "id": "OSP-02",
    "number": "02",
    "title": "Contributor Experience And CI",
    "status": "passed",
    "type": "implementation",
    "repo_path": ".",
    "docs_path": "docs/open_source_platform_optimization",
    "phase_file": "docs/open_source_platform_optimization/phase-02-contributor-experience-and-ci.md",
    "depends_on": [
      "OSP-01"
    ],
    "unlocks": [
  ...
  "goal": {
    "target": "Complete the Contributor Experience And CI slice while preserving prior phase contracts and downstream handoff boundaries.",
    "prompt": "Complete OSP-02 Contributor Experience And CI for `.` by following `docs/open_source_platform_optimization/phase-02-contributor-experience-and-ci.md`; work on the matching feature-oracle item, preserve continuity with adjacent phases, write code facts back to the source packet and continuity ledger, stay inside the named edit boundaries, make the smallest requirement-satisfying change, and finish only after validation, regression, review, compliance, rollback, evidence, and acceptance gates pass or blockers are documented.",
    "plan_required": true,
    "plan_output": "docs/open_source_platform_optimization/reports/osp-02-contributor-experience-and-ci-plan.md",
    "completion_report": "docs/open_source_platform_optimization/reports/osp-02-contributor-experience-and-ci-report.md"
  },
  "runtime": {
  ...
  "context": {
    "read_first": [
      ...
      "docs/open_source_platform_optimization/phase-02-contributor-experience-and-ci.md"
    ],
    "primary_context": [
      "docs/open_source_platform_optimization/README.md",
      "docs/open_source_platform_optimization/source-packet.md",
  ...
  "validation": {
    "commands": [
      {
        "id": "ci-workflow-review",
        "cwd": ".",
        "command": "sed -n '1,240p' .github/workflows/ci.yml && sed -n '1,220p' .github/workflows/docker-publish.yml && sed -n '1,220p' .github/workflows/publish-sdk.yml",
        "expected": "Existing CI, Docker publish, and SDK publish workflows are reviewed before edits.",
        "required": true
      },
      {
        "id": "contributor-core-checks",
        "cwd": ".",
        "command": "bash -n scripts/new/validate-env.sh scripts/new/seed-demo-data.sh && uv run ruff check tests/scripts/test_validate_env_quickstart.py tests/scripts/test_web_runtime_config_entrypoint.py tests/scripts/test_seed_demo_data.py && uv run --extra dev --extra test pytest -q --no-cov tests/scripts/test_validate_env_quickstart.py tests/scripts/test_web_runtime_config_entrypoint.py tests/scripts/test_seed_demo_data.py && docker compose --env-file .env.example config --quiet && pnpm -C web type-check && pnpm -C web lint && pnpm -C web build",
        "expected": "Shell syntax, focused Python lint/tests, compose static config, and frontend contributor checks pass or blockers are recorded.",
        "required": true
      },
      {
        ...
        "expected": "OSP harness remains structurally valid after CI and contributor-experience updates.",
        "required": true
      }
    ],
    "browser_checks": [
  ...
  "evidence": {
    "outputs": [
      "docs/open_source_platform_optimization/reports/osp-02-contributor-experience-and-ci-report.md"
    ],
    "required_artifacts": [
      "phase report",
      "progress-log entry",
...
- PHASE_ID: OSP-02
- GOAL_TARGET: Complete the Contributor Experience And CI slice while preserving prior phase contracts and downstream handoff boundaries.
- GOAL_PROMPT: Complete OSP-02 Contributor Experience And CI for `.` by following `docs/open_source_platform_optimization/phase-02-contributor-experience-and-ci.md`; work on feature-oracle item OSP-F003; preserve dependency continuity with OSP-01; write code facts and boundary decisions back before handoff; stay inside the named edit boundaries; make the smallest requirement-satisfying change; finish only after validation, regression, review, compliance, rollback, evidence, and acceptance gates pass or blockers are documented.
- DEPENDS_ON: OSP-01
...
- VALIDATION_COMMANDS: sed -n '1,240p' .github/workflows/ci.yml; bash -n scripts/new/validate-env.sh scripts/new/seed-demo-data.sh; uv run ruff check tests/scripts/test_validate_env_quickstart.py tests/scripts/test_web_runtime_config_entrypoint.py tests/scripts/test_seed_demo_data.py; uv run --extra dev --extra test pytest -q --no-cov tests/scripts/test_validate_env_quickstart.py tests/scripts/test_web_runtime_config_entrypoint.py tests/scripts/test_seed_demo_data.py; docker compose --env-file .env.example config --quiet; pnpm -C web type-check; pnpm -C web lint; pnpm -C web build; python3 /Users/misaya.yanghejazfs.com.au/.codex/skills/prd-phase-harness/scripts/validate_harness_prd.py docs/open_source_platform_optimization --strict --quality-score
...
- EVIDENCE_OUTPUT: `docs/open_source_platform_optimization/reports/osp-02-contributor-experience-and-ci-report.md`
...
Execute OSP-02 by using the phase contract, updating OSP-F003, and preserving the dependency chain `OSP-00 Open Source Baseline Audit
...
- Depends on: OSP-01
- Unlocks: OSP-03
- Feature-oracle item: OSP-F003
- Continuity ledger: `docs/open_source_platform_optimization/continuity-ledger.md`
- Prior-phase evidence to inherit: the OSP-01 phase report, progress-log entry, oracle evidence, and continuity-ledger boundary notes
...
- Work needed to satisfy OSP-F003 for Contributor Experience And CI.
...
- Start with this phase file, `source-packet.md`, `continuity-ledger.md`, and the OSP-01 phase report.
...
OSP-02 must update OSP-F003, produce durable evidence, and write code/interface facts back so the next phase can continue without hidden chat context.
...
- OSP-F003 has evidence or a documented blocker.
- `docs/open_source_platform_optimization/reports/osp-02-contributor-experience-and-ci-report.md` exists or is named as blocked evidence.
