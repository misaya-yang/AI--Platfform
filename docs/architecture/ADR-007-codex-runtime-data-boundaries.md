# ADR-007: Codex Runtime model, state, and capability boundaries
...
**Scope:** Gateway model plane, Codex Runtime service, PostgreSQL ThreadStore, and Assistant capability service

## Context

Codex currently speaks the Responses wire protocol through a replaceable
`ModelProvider`. The platform's public `/v1/responses` endpoint is an
Agent-level compatibility API backed by the Python loop; pointing Codex at that
...
The Codex Runtime creates and stores an immutable `AgentRuntimeSnapshotV1`
...
`PostgresThreadStore` is the authority for Codex state:

- `assistant_session_runtime_assignments` immutably selects Python control or
  the pinned Codex candidate for one tenant/user/session before any Turn;
- `assistant_runtime_threads` binds one tenant/user/session to one Harness
...
- `assistant_runs` records Harness Thread/Turn, kernel, snapshot, capability,
...
call a model or own a tool-selection loop. Codex accesses capabilities through
...
Gateway and Web remain the only public services. Codex Runtime, capability
service, Knowledge, PostgreSQL, Redis, Qdrant, MCP children, and sandboxes stay
private to the deployment network. The candidate Runtime uses a separate
internal endpoint during canary; stable session assignment is persisted and is
never chosen from user text.

Before the private model plane exists, Codex-assigned Turn requests fail
locally with `CODEX_RUNTIME_TURNS_NOT_READY`; they never fall through to the
Python loop. The optional Compose `codex-runtime` profile is therefore a
lifecycle/recovery candidate only during Phase 1.

## Rollback

New-session assignment can be switched back to Python without moving active
Turns. A legacy projector keeps quiescent Codex sessions resumable by the
