...
class SSEParserTest {
    ...
    void exceptionHierarchy() {
        ...
        assertInstanceOf(HejazAIException.class, auth);
        assertEquals(401, auth.getStatusCode());

        var rate = new RateLimitException("slow down", 429, null, 30.0);
        assertInstanceOf(HejazAIException.class, rate);
        assertEquals(30.0, rate.getRetryAfterSeconds());

        var stream = new StreamException("parse error");
        assertInstanceOf(HejazAIException.class, stream);

        // fromStatus factory
        assertInstanceOf(AuthException.class, HejazAIException.fromStatus(401, "unauth", null));
        assertInstanceOf(AuthException.class, HejazAIException.fromStatus(403, "forbidden", null));
        assertInstanceOf(RateLimitException.class, HejazAIException.fromStatus(429, "limited", null));
    }
}
