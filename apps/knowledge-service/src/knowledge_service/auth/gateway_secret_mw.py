...
Closes Polaris item #6-KB — any sibling container on the Docker bridge
network could previously craft ``X-User-Id``/``X-Tenant-Id`` headers and
call knowledge-service directly, impersonating arbitrary users. With this
middleware enabled (and ``allow_anonymous=false``) knowledge-service
refuses traffic that did not pass through the gateway signer.

Design reuses :mod:`ai_gateway_core.auth.gateway_secret` verbatim — the
same HMAC/verify/replay code assistant-service uses. This module is a
thin ASGI wrapper; **it MUST NOT fork the crypto**.
...
``/health`` (and ``/metrics``) are always allowed so orchestrators can
probe without a secret — matches assistant-service behaviour.
"""
