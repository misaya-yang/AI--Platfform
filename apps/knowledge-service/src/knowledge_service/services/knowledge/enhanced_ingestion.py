...
class EnhancedIngestionPipeline:
    ...
    async def process_document(
        ...
        mode: str | None = None,  # "simple", "structured", "auto"
    ) -> dict[str, Any]:
        """
        ...
        if mode == "structured" and self.parser and filename.lower().endswith(".pdf"):
            return await self._process_structured(dataset_id, document_id, content_bytes, filename)
        else:
            return await self._process_simple(
                dataset_id, document_id, content_bytes, filename, mime_type
            )

    def _determine_best_mode(self, filename: str, mime_type: str | None) -> str:
    ...
    async def _convert_to_kb_chunk(
        ...
        if chunk.type == ChunkType.TEXT:
            ...
            if image_data:
                ...
                storage_key = None
                if (
                    hasattr(self.kb_service, "image_storage_service")
                    and self.kb_service.image_storage_service
                ):
                    try:
