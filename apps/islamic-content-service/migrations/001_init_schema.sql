CREATE SCHEMA IF NOT EXISTS __ISLAMIC_CONTENT_SCHEMA__;
...
CREATE TABLE IF NOT EXISTS quran_ayah_translations (
    id BIGSERIAL PRIMARY KEY,
    verse_key VARCHAR(16) NOT NULL REFERENCES quran_ayahs(verse_key) ON DELETE CASCADE,
    translation_id INTEGER NOT NULL REFERENCES quran_translations(translation_id) ON DELETE CASCADE,
    translation_text TEXT NOT NULL DEFAULT '',
    source_api VARCHAR(64) NOT NULL DEFAULT 'quran.foundation',
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX IF NOT EXISTS idx_quran_ayah_translations_lookup
    ON quran_ayah_translations(translation_id, verse_key);

CREATE TABLE IF NOT EXISTS quran_ayah_audio (
    id BIGSERIAL PRIMARY KEY,
    verse_key VARCHAR(16) NOT NULL REFERENCES quran_ayahs(verse_key) ON DELETE CASCADE,
    recitation_id INTEGER NOT NULL REFERENCES quran_recitations(recitation_id) ON DELETE CASCADE,
    audio_url TEXT,
    source_api VARCHAR(64) NOT NULL DEFAULT 'quran.foundation',
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (verse_key, recitation_id)
);

CREATE INDEX IF NOT EXISTS idx_quran_ayah_audio_lookup
    ON quran_ayah_audio(recitation_id, verse_key);

CREATE TABLE IF NOT EXISTS quran_chapter_audio_tracks (
    id BIGSERIAL PRIMARY KEY,
    chapter_id INTEGER NOT NULL REFERENCES quran_chapters(chapter_id) ON DELETE CASCADE,
    recitation_id INTEGER NOT NULL REFERENCES quran_recitations(recitation_id) ON DELETE CASCADE,
    audio_url TEXT,
    source_api VARCHAR(64) NOT NULL DEFAULT 'quran.foundation',
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (chapter_id, recitation_id)
);

CREATE INDEX IF NOT EXISTS idx_quran_chapter_audio_tracks_lookup
    ON quran_chapter_audio_tracks(chapter_id, recitation_id);

CREATE TABLE IF NOT EXISTS quran_chapter_audio_timings (
    id BIGSERIAL PRIMARY KEY,
    chapter_id INTEGER NOT NULL REFERENCES quran_chapters(chapter_id) ON DELETE CASCADE,
    recitation_id INTEGER NOT NULL,
    verse_key VARCHAR(16) NOT NULL,
    timestamp_from_ms INTEGER,
    timestamp_to_ms INTEGER,
    duration_ms INTEGER,
    segments_json JSONB NOT NULL DEFAULT '[]'::jsonb,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (recitation_id, verse_key)
);

CREATE INDEX IF NOT EXISTS idx_quran_audio_timings_chapter_verse
    ON quran_chapter_audio_timings(chapter_id, recitation_id, verse_key);

CREATE TABLE IF NOT EXISTS quran_triplet_ranges (
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (chapter_id, start_ayah_number, end_ayah_number)
);

CREATE INDEX IF NOT EXISTS idx_quran_triplet_ranges_lookup
    ON quran_triplet_ranges(chapter_id, start_ayah_number, end_ayah_number);
...
CREATE INDEX IF NOT EXISTS idx_hadith_localizations_item_language
    ON hadith_localizations(hadith_item_id, language);
