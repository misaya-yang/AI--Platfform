...
A Codex-backed candidate service can persist and resume ordered Thread/Turn/Item state while producing the existing V1 stream contract.
...
| --- | --- | --- |
| ThreadStore contract | `make codex-thread-store-contract` | A second Runtime instance resumes gap-free, non-duplicated Items and V1 projection. |

## Checkpoint 1 (2026-08-21)

Completed:

- Narrow Codex host injection seams for ThreadStore, typed extensions, and a
...
- A second Codex Runtime instance resumes the same PostgreSQL Thread, then
  ...
  rollout path.
- Immutable, prompt-agnostic session runtime assignments in Gateway. A Codex
  assignment cannot silently fall through to the Python control loop before
  Phase 2 Turn routing exists.
- Optional `codex-runtime` Compose profile and dedicated Rust Runtime image
...
- Added clean-fork Runtime image construction and an isolated Docker smoke that
  uses an internal-only network, ephemeral PostgreSQL, isolated `CODEX_HOME`,
...
- `make codex-thread-store-contract`: Python 4 passed, Rust 6 passed, live
...
- Codex Turn assignments remain fail-closed until Phase 2 implements and
...
- Added a fail-closed container bootstrap marker before Codex `arg0` creates its
...
- `make codex-runtime-contract`: both artifact identities and digests passed.
- `CODEX_RUNTIME_IMAGE=ai-gateway-codex-agent-runtime:local-44d926ab7c9e make codex-runtime-smoke`:
