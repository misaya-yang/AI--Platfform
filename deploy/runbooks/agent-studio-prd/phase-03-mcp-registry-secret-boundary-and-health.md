...
```json
{
  ...
  "validation": {
    "commands": [
      {"id": "mcp-api-runtime", "cwd": ".", "command": "uv run pytest -q --no-cov tests/api/test_mcp_registry_api.py && uv run --package assistant-service pytest -q --no-cov tests/services/assistant/test_mcp_runtime.py tests/services/assistant/tools/test_mcp_capability_policy.py tests/services/assistant/test_connector_credential_principal.py", "expected": "Tenant CRUD, redaction, service-account/user-delegated grants, current-user resolution, anonymous/public denial, Connector parity, discovery, Agent binding, disable/revoke, timeout, concurrency and circuit tests pass against local mocks.", "required": true},
      {"id": "mcp-security", "cwd": ".", "command": "uv run pytest -q --no-cov tests/security/test_mcp_security.py", "expected": "Private/link-local/metadata targets, unsafe redirects, DNS rebinding, bad Origin, wrong OAuth audience, cross-tenant token use, oversized responses, token leakage and session confusion are rejected.", "required": true},
      {"id": "lint", "cwd": ".", "command": "uv run ruff check apps/assistant-service/src/assistant_service/core/mcp apps/assistant-service/src/assistant_service/core/tool_invoker.py src/api/v1/mcp.py src/api/schemas/mcp.py packages/ai-gateway-core/src/ai_gateway_core/persistence/repositories/mcp_repository.py tests/api/test_mcp_registry_api.py tests/services/assistant/test_mcp_runtime.py tests/security/test_mcp_security.py", "expected": "Ruff exits zero for MCP implementation and tests.", "required": true},
