...
```json
{
  ...
  "validation": {
    "commands": [
      {"id": "operations-governance", "cwd": ".", "command": "uv run pytest -q --no-cov tests/api/test_agent_observability.py tests/security/test_agent_data_governance.py tests/database/test_agent_studio_migrations.py", "expected": "Trace/metric filters, redaction, audit, quota, archive, token/grant revocation, retention, user/tenant deletion, legal hold, composite tenant constraints and migration idempotency/compatibility tests pass.", "required": true},
