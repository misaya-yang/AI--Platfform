# Phase 05 - Operating Model And Release Gate
...
**Goal:** Publish the Assistant runtime operating model, read-only doctor/status plan, terminology invariants, no-go thresholds, waiver policy, and stable offline release gate path.

**Architecture:** Keep `make verify-eval-dev` as the known eval baseline. Add or document a future Assistant runtime regression gate only after AHR-01 through AHR-04 produce stable offline tests and reports. OpenClaw doctor/status informs the read-only diagnostic shape, but auto-repair, migration, and production mutation remain out of scope unless a later phase explicitly approves them.

**Tech Stack:** Makefile gates, eval golden scripts, assistant-service pytest, web pnpm gates, runbook reports, CI workflow only after gate stability.
...
```json
{
  ...
  "phase": {
    "id": "AHR-05",
    "number": "05",
    "title": "Operating Model And Release Gate",
    "status": "ready",
    "type": "implementation",
    "repo_path": ".",
    "docs_path": "deploy/runbooks/assistant-hermes-runtime-prd",
    "phase_file": "deploy/runbooks/assistant-hermes-runtime-prd/phase-05-operating-model-and-release-gate.md",
    "depends_on": [
      "AHR-04"
    ],
    "unlocks": []
  ...
  "goal": {
    "target": "Complete the Operating Model, read-only doctor/status, terminology invariant, and Release Gate slice while preserving prior phase contracts and downstream handoff boundaries.",
    "prompt": "Complete AHR-05 Operating Model And Release Gate for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-05-operating-model-and-release-gate.md`; work on the matching feature-oracle item, preserve continuity with adjacent phases, write code facts, doctor/status design, terminology invariants, and release gate decisions back to the source packet and continuity ledger, stay inside the named edit boundaries, make the smallest requirement-satisfying change, and finish only after validation, regression, review, compliance, rollback, evidence, and acceptance gates pass or blockers are documented.",
    "plan_required": true,
    "plan_output": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-05-operating-model-and-release-gate-plan.md",
    "completion_report": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-05-operating-model-and-release-gate-report.md"
  },
  "runtime": {
  ...
  "context": {
    "read_first": [
      ...
      "deploy/runbooks/assistant-hermes-runtime-prd/phase-05-operating-model-and-release-gate.md"
    ],
    "primary_context": [
  ...
  "validation": {
    "commands": [
      {
        "id": "assistant-runtime-release-gate",
        "cwd": ".",
        "command": "make verify-eval-dev && make eval-regression-gate",
        "expected": "Eval dev verification and golden regression gate pass; any future assistant runtime gate and doctor/status output are offline, deterministic, read-only, redacted, and reported before CI adoption.",
        "required": true
      }
    ],
    "browser_checks": [
  ...
  "evidence": {
    "outputs": [
      "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-05-operating-model-and-release-gate-report.md"
    ],
    "required_artifacts": [
      "phase report",
      "progress-log entry",
...
- PHASE_ID: AHR-05
- GOAL_TARGET: Complete the Operating Model, read-only doctor/status, terminology invariant, and Release Gate slice while preserving prior phase contracts and downstream handoff boundaries.
- GOAL_PROMPT: Complete AHR-05 Operating Model And Release Gate for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-05-operating-model-and-release-gate.md`; work on feature-oracle item AHR-F006; preserve dependency continuity with AHR-04; write code facts, doctor/status design, terminology invariants, and release gate decisions back before handoff; stay inside the named edit boundaries; make the smallest requirement-satisfying change; finish only after validation, regression, review, compliance, rollback, evidence, acceptance gates, and `--completion-gate --phase AHR-05` pass or blockers are documented.
- DEPENDS_ON: AHR-04
...
- VALIDATION_COMMANDS: make verify-eval-dev && make eval-regression-gate
...
- REGRESSION_SCOPE: Prior phase report evidence, feature-oracle status, and continuity-ledger boundaries remain valid. if this is the terminal phase or release gate, whole-demand regression across completed feature-oracle items is executed or blocked with evidence.
...
- ACCEPTANCE_GATES: Phase report exists; validation or blocker evidence is recorded; oracle item, progress log, handoff, source packet, read-only doctor/status plan, terminology invariants, release gate no-go thresholds, and continuity ledger are updated; minimal-change scope and independent critic verdict are recorded. Terminal phase performs whole-demand regression or records an explicit blocker.
- EVIDENCE_OUTPUT: `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-05-operating-model-and-release-gate-report.md`
...
10. Run `--strict --completion-gate --phase AHR-05` before claiming this phase is passed or unlocked.
...
Execute AHR-05 by using the phase contract, updating AHR-F006, and preserving the dependency chain `AHR-00 Comparative Baseline Evidence
...
- Prior-phase evidence to inherit: the AHR-04 phase report, progress-log entry, oracle evidence, and continuity-ledger boundary notes
...
- Work needed to satisfy AHR-F006 for Operating Model And Release Gate.
...
- Open only the AHR-04 report summary or continuity row needed to confirm inherited boundaries.
...
Publish a runbook that explains normal runtime health, failure categories, no-go thresholds, rollback behavior, owner surfaces, report locations, and waiver policy.

### R2 Offline Runtime Regression Gate

Define or dry-run a local offline command such as `make verify-assistant-runtime-dev` only after it is stable. The gate should cover turn envelope, stream parity, approval/resume, tool safety, safe_fetch wiring, sandbox failure, memory completed-turn sync, compaction lineage, and eval golden regression.

### R3 Report Output

When implemented, the gate should write JSON and Markdown outputs under `reports/assistant-runtime-regression/` with failed case, expected behavior, actual behavior, owner phase, and redaction status.

### R4 CI Adoption Policy

Do not add an unstable runtime gate to required PR CI. First make it reproducible locally, then optional/manual, then CI-blocking only after flake and dependency risks are understood.

### R5 Whole-Demand Regression

Terminal completion must run whole-demand regression across all feature-oracle items marked passing or record explicit blockers for any missing phase evidence.

## Test and Regression Requirements

Candidate commands to confirm or refine during AHR-00 and prior phases:
...
- AHR-F006 has evidence or a documented blocker.
- `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-05-operating-model-and-release-gate-report.md` exists or is named as blocked evidence.
...
- No production deployment, production migration, or live provider load test is performed without explicit approval.
- Runtime gate adoption policy is documented before CI changes.
