# Phase 02 - Memory Context And Compaction Lineage
...
**Goal:** Harden Assistant memory and context lifecycle with Hermes-inspired write discipline plus OpenClaw-inspired workspace memory enumeration, transcript separation, pre-compaction flush, completed-turn sync, profile separation, and compaction lineage.

**Architecture:** Preserve AI--Platfform's DB-backed multi-tenant memory model. Apply Hermes concepts at the lifecycle and safety boundary: atomic writes, dedup, stable user profile source, provider hooks, completed-turn-only sync, snippet provenance, and parent/child compaction lineage. Apply OpenClaw concepts at the layer boundary: workspace memory source enumeration, session transcript as replay state, durable memory as curated or lifecycle-synced state, checkpoint as resume state, trace as observability, retrieval index as derived state, and pre-compaction flush with skip/failure evidence.

**Tech Stack:** `assistant_service.core.runtime.memory`, `runtime_adapter.py`, `runtime_memory.py`, checkpoint metadata, memory migrations, pytest.
...
```json
{
  ...
  "phase": {
    "id": "AHR-02",
    "number": "02",
    "title": "Memory Context And Compaction Lineage",
    "status": "ready",
    "type": "implementation",
    "repo_path": ".",
    "docs_path": "deploy/runbooks/assistant-hermes-runtime-prd",
    "phase_file": "deploy/runbooks/assistant-hermes-runtime-prd/phase-02-memory-context-and-compaction-lineage.md",
    "depends_on": [
      "AHR-01"
    ],
    "unlocks": [
  ...
  "goal": {
    "target": "Complete the Memory Context, transcript separation, workspace source enumeration, pre-compaction flush, and Compaction Lineage slice while preserving prior phase contracts and downstream handoff boundaries.",
    "prompt": "Complete AHR-02 Memory Context And Compaction Lineage for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-02-memory-context-and-compaction-lineage.md`; work on the matching feature-oracle item, preserve continuity with adjacent phases, write code facts and memory-layer boundary decisions back to the source packet and continuity ledger, stay inside the named edit boundaries, make the smallest requirement-satisfying change, and finish only after validation, regression, review, compliance, rollback, evidence, and acceptance gates pass or blockers are documented.",
    "plan_required": true,
    "plan_output": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-02-memory-context-and-compaction-lineage-plan.md",
    "completion_report": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-02-memory-context-and-compaction-lineage-report.md"
  },
  "runtime": {
  ...
  "context": {
    "read_first": [
      ...
      "deploy/runbooks/assistant-hermes-runtime-prd/phase-02-memory-context-and-compaction-lineage.md"
    ],
    "primary_context": [
  ...
  "validation": {
    "commands": [
      {
        "id": "assistant-memory-lineage-tests",
        "cwd": ".",
        "command": "uv run --package assistant-service pytest -q --no-cov tests/services/assistant",
        "expected": "Assistant memory, context, transcript separation, workspace source enumeration, pre-compaction flush, checkpoint, and lineage tests pass after this phase adds focused coverage.",
        "required": true
      }
    ],
    "browser_checks": [
  ...
  "evidence": {
    "outputs": [
      "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-02-memory-context-and-compaction-lineage-report.md"
    ],
    "required_artifacts": [
      "phase report",
      "progress-log entry",
...
- PHASE_ID: AHR-02
- GOAL_TARGET: Complete the Memory Context, transcript separation, workspace source enumeration, pre-compaction flush, and Compaction Lineage slice while preserving prior phase contracts and downstream handoff boundaries.
- GOAL_PROMPT: Complete AHR-02 Memory Context And Compaction Lineage for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-02-memory-context-and-compaction-lineage.md`; work on feature-oracle item AHR-F003; preserve dependency continuity with AHR-01; write code facts and memory-layer boundary decisions back before handoff; stay inside the named edit boundaries; make the smallest requirement-satisfying change; finish only after validation, regression, review, compliance, rollback, evidence, acceptance gates, and `--completion-gate --phase AHR-02` pass or blockers are documented.
- DEPENDS_ON: AHR-01
...
- ACCEPTANCE_GATES: Phase report exists; validation or blocker evidence is recorded; oracle item, progress log, handoff, source packet, workspace memory enumeration, transcript/durable memory separation, pre-compaction flush behavior, and continuity ledger are updated; minimal-change scope and independent critic verdict are recorded.
- EVIDENCE_OUTPUT: `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-02-memory-context-and-compaction-lineage-report.md`
...
10. Run `--strict --completion-gate --phase AHR-02` before claiming this phase is passed or unlocked.
...
Execute AHR-02 by using the phase contract, updating AHR-F003, and preserving the dependency chain `AHR-00 Comparative Baseline Evidence
...
- Prior-phase evidence to inherit: the AHR-01 phase report, progress-log entry, oracle evidence, and continuity-ledger boundary notes
...
- Work needed to satisfy AHR-F003 for Memory Context And Compaction Lineage.
...
- Open only the AHR-01 report summary or continuity row needed to confirm inherited boundaries.
...
### R1 Memory Source Write Safety

Memory source writes must be transactional or atomic, bounded, deduplicated, drift-aware, and threat-scanned. If AI--Platfform's DB pattern makes file locks irrelevant, use row-level transaction semantics instead of copying Hermes file behavior.

### R2 Stable Profile Separation

Stable user preferences should be separated from daily/reflection/project memory, either through USER.md-equivalent sources or first-class DB source types. The phase must preserve tenant/user boundaries and sensitivity metadata.

### R3 Memory Provider Lifecycle

Define a lifecycle contract for initialize, prefetch, sync_turn, on_session_end, on_session_switch, on_pre_compact, on_memory_write, and flush_pending. The default behavior must be safe when providers fail.

### R4 Completed-Turn-Only Sync

Interrupted, cancelled, failed, approval-pending, or denied turns must not sync into long-term memory by default. The behavior must be tested.

### R5 Compaction Lineage And Context Provenance

Compaction/resume must record parent/child lineage and summary provenance. Injected memory snippets must include source type, source id/path, score, recency, and untrusted-context marking.
...
Add focused tests for concurrent memory writes, duplicate facts, provider failure, interrupted-turn skip, context snippet sanitization/provenance, and compaction resume lineage.
...
- AHR-F003 has evidence or a documented blocker.
- `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-02-memory-context-and-compaction-lineage-report.md` exists or is named as blocked evidence.
...
- Long-term memory is not polluted by failed, interrupted, denied, or approval-pending turns.
- Snippet provenance is available for trace/eval and future UI without leaking sensitive content.
