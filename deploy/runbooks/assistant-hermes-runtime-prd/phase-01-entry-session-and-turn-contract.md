# Phase 01 - Entry Session And Turn Contract
...
**Goal:** Add a durable Assistant run/session/turn contract plus context compiler snapshot so every turn can explain how it started, what capabilities were actually available, what happened, why it stopped, and how it can be resumed or evaluated.

**Architecture:** Build on existing Assistant API routes, `AgentLoop`, stream event mappings, trace writer, and `AssistantExecutionGateway` checkpoint/resume contracts. Add an OpenClaw-inspired context compiler boundary that records tool registry, policy, sandbox, memory, workspace, provider/model, bootstrap, and surface facts used to assemble model input. Keep all public shapes additive.

**Tech Stack:** Python assistant-service, FastAPI routes, stream event schemas, gateway checkpoint/resume services, Eval trace contracts, pytest.
...
```json
{
  ...
  "phase": {
    "id": "AHR-01",
    "number": "01",
    "title": "Entry Session And Turn Contract",
    "status": "ready",
    "type": "implementation",
    "repo_path": ".",
    "docs_path": "deploy/runbooks/assistant-hermes-runtime-prd",
    "phase_file": "deploy/runbooks/assistant-hermes-runtime-prd/phase-01-entry-session-and-turn-contract.md",
    "depends_on": [
      "AHR-00"
    ],
    "unlocks": [
  ...
  "goal": {
    "target": "Complete the Entry Session And Turn Contract plus context compiler slice while preserving prior phase contracts and downstream handoff boundaries.",
    "prompt": "Complete AHR-01 Entry Session And Turn Contract for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-01-entry-session-and-turn-contract.md`; work on the matching feature-oracle item, preserve continuity with adjacent phases, write code facts and context compiler boundary decisions back to the source packet and continuity ledger, stay inside the named edit boundaries, make the smallest requirement-satisfying change, and finish only after validation, regression, review, compliance, rollback, evidence, and acceptance gates pass or blockers are documented.",
    "plan_required": true,
    "plan_output": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-01-entry-session-and-turn-contract-plan.md",
    "completion_report": "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-01-entry-session-and-turn-contract-report.md"
  },
  "runtime": {
  ...
  "context": {
    "read_first": [
      ...
      "deploy/runbooks/assistant-hermes-runtime-prd/phase-01-entry-session-and-turn-contract.md"
    ],
    "primary_context": [
  ...
  "validation": {
    "commands": [
      {
        "id": "assistant-runtime-contract-tests",
        "cwd": ".",
        "command": "uv run --package assistant-service pytest -q --no-cov tests/services/assistant",
        "expected": "Assistant runtime tests pass, including any added run/session/turn contract, context compiler snapshot, stream/non-stream parity, and prompt/runtime capability truthfulness coverage.",
        "required": true
      }
    ],
    "browser_checks": [
  ...
  "evidence": {
    "outputs": [
      "deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-01-entry-session-and-turn-contract-report.md"
    ],
    "required_artifacts": [
      "phase report",
      "progress-log entry",
...
- PHASE_ID: AHR-01
- GOAL_TARGET: Complete the Entry Session And Turn Contract plus context compiler slice while preserving prior phase contracts and downstream handoff boundaries.
- GOAL_PROMPT: Complete AHR-01 Entry Session And Turn Contract for `.` by following `deploy/runbooks/assistant-hermes-runtime-prd/phase-01-entry-session-and-turn-contract.md`; work on feature-oracle item AHR-F002; preserve dependency continuity with AHR-00; write code facts and context compiler boundary decisions back before handoff; stay inside the named edit boundaries; make the smallest requirement-satisfying change; finish only after validation, regression, review, compliance, rollback, evidence, acceptance gates, and `--completion-gate --phase AHR-01` pass or blockers are documented.
- DEPENDS_ON: AHR-00
...
- ACCEPTANCE_GATES: Phase report exists; validation or blocker evidence is recorded; oracle item, progress log, handoff, source packet, context compiler snapshot contract, and continuity ledger are updated; minimal-change scope and independent critic verdict are recorded.
- EVIDENCE_OUTPUT: `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-01-entry-session-and-turn-contract-report.md`
...
10. Run `--strict --completion-gate --phase AHR-01` before claiming this phase is passed or unlocked.
...
Execute AHR-01 by using the phase contract, updating AHR-F002, and preserving the dependency chain `AHR-00 Comparative Baseline Evidence
...
- Depends on: AHR-00
- Unlocks: AHR-02
- Feature-oracle item: AHR-F002
- Continuity ledger: `deploy/runbooks/assistant-hermes-runtime-prd/continuity-ledger.md`
- Prior-phase evidence to inherit: the AHR-00 phase report, progress-log entry, oracle evidence, and continuity-ledger boundary notes
...
- Work needed to satisfy AHR-F002 for Entry Session And Turn Contract.
...
- Open only the AHR-00 report summary or continuity row needed to confirm inherited boundaries.
...
### R1 Run And Session Envelope

Every Assistant run should have a stable run id, session id, thread id when available, status, tenant/user scope, started/ended timestamps, model/provider snapshot, checkpoint id, and trace id when available.

### R2 Turn Exit Reasons

Every terminal turn state should record an explicit reason: succeeded, interrupted, approval_pending, denied, cancelled, tool_error, model_error, max_iterations, checkpoint_resume_ready, or failed. If existing code uses different names, AHR-01 must map them without breaking current callers.

### R3 Stream And Non-Stream Parity

Stream and non-stream code paths must agree on terminal envelope fields. Future Eval/UI should be able to consume the same terminal state without scraping text.

### R4 Stop, Approval, And Resume Semantics

Stop/cancel, approval pending, approval resolved, and resume-plan paths must be represented as structured state and tied to checkpoint/trace evidence.
...
Add focused tests for stream/non-stream parity, stop/cancel, approval pending, tool error, max-iteration exit, and resume-plan state before claiming this phase passed.
...
- AHR-F002 has evidence or a documented blocker.
- `deploy/runbooks/assistant-hermes-runtime-prd/reports/ahr-01-entry-session-and-turn-contract-report.md` exists or is named as blocked evidence.
...
- Existing API callers remain compatible; new fields are additive.
- Trace/eval consumers can access the terminal envelope without parsing assistant prose.
