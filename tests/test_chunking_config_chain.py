...
def test_chunking_config_from_dict():
    ...
    assert result["use_token_count"] == True
...
def test_hierarchical_indexer_config_merge():
    ...
    child_size = chunking_config.get('child_chunk_size') or \
                 chunking_config.get('chunk_size') or \
                 l3_chunk_size
    
    child_overlap = chunking_config.get('child_overlap') or \
                   chunking_config.get('chunk_overlap') or \
                   l3_chunk_overlap
    
    parent_size = chunking_config.get('parent_chunk_size') or \
                  (child_size * 4)
    parent_overlap = chunking_config.get('parent_overlap') or \
                    (child_overlap * 2)
    ...
    assert result_config["use_token_count"] == True
