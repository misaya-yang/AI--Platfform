...
from scripts.harness import codex_runtime_text_gate as text_gate
...
def _clear_provider_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    for name in (
        ...
        "DASHSCOPE_BASE_URL",
        "CODEX_RUNTIME_TEXT_TTFT_LIMIT_SECONDS",
        "CODEX_RUNTIME_LONG_OUTPUT_MIN_CHARS",
    ):
...
def test_gate_config_reads_runtime_only_provider_inputs(
    ...
    env_file.write_text(
        ...
        "CODEX_RUNTIME_TEXT_TTFT_LIMIT_SECONDS=5\n"
        "CODEX_RUNTIME_LONG_OUTPUT_MIN_CHARS=800\n",
        encoding="utf-8",
    )
