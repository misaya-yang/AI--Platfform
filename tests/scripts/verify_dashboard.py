    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=30.0, verify=False) as client:
        ...
        try:
            ...
            if not user2:
                ...
                create_resp = await client.post("/v1/users", headers=headers, json={
            ...
            if login_resp.status_code == 400 and "change password" in login_resp.text.lower():
                 ...
                 print("User 2 needs password change. Skipping specific User 2 login, using Admin for traffic simulation with User 2 context if possible.")
        ...
        if user2_token:
            ...
            try:
                ...
                resp = await client.post("/api/v1/assistant/chat", headers=user2_headers, json=chat_payload)
                if resp.status_code == 200:
        ...
        try:
            # Summary
            summary_resp = await client.get("/api/v1/dashboard/summary?period=today", headers=headers)
            ...
            if user2 and user2.get("user_id"):
                 user_stats = await client.get(f"/api/v1/dashboard/user/{user2['user_id']}", headers=headers)
