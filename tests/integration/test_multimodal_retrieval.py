...
class MultimodalRetrievalTester:
    ...
    async def create_multimodal_dataset(self, name: str = "Multimodal Test KB") -> Dict[str, Any]:
        ...
        resp = await self.client.post(
            f"{self.base_url}/api/v1/knowledge/datasets",
            json=payload
        )
    ...
    async def upload_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        with open(pdf_file, "rb") as f:
            ...
            resp = await self.client.post(
                f"{self.base_url}/api/v1/knowledge/{self.dataset_id}/documents/upload",
                files=files
            )
    ...
    async def get_document_segments(self, document_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if doc_id:
            ...
            for doc in docs[:5]:  # Limit to first 5 documents
                ...
                if doc_resp.status_code == 200:
                    ...
                    segs = seg_result if isinstance(seg_result, list) else seg_result.get("segments", [])
        ...
        for seg in segments:
            ...
            if content_type == "image":
                ...
                analysis["image_details"].append({
                    ...
                    "vlm_description": seg.get("vlm_description", "")[:200] if seg.get("vlm_description") else None,
    ...
    async def retrieve(
        ...
        resp = await self.client.post(
            f"{self.base_url}/api/v1/knowledge/{self.dataset_id}/retrieve",
            json=payload
        )
    ...
    async def run_test_case(self, test_case: TestCase) -> Dict[str, Any]:
        ...
        def analyze_results(results: List[Dict]) -> Dict[str, Any]:
            return {
                ...
                "keywords_found": [
                    ...
                    if any(kw.lower() in (r.get("text", "") + str(r.get("vlm_description", ""))).lower()
                           for r in results)
                ],
        ...
        # Evaluate test case
        found_image = (
            standard_analysis["image_hits"] > 0 or
            reranked_analysis["image_hits"] > 0 or
            standard_analysis["with_associated_images"] > 0
        )

        keywords_coverage = len(standard_analysis["keywords_found"]) / len(test_case.expected_keywords) if test_case.expected_keywords else 1.0
        ...
        logger.info(f"  Standard: {standard_analysis['total_hits']} hits ({standard_analysis['text_hits']} text, {standard_analysis['image_hits']} image)")
        logger.info(f"  Reranked: {reranked_analysis['total_hits']} hits ({reranked_analysis['text_hits']} text, {reranked_analysis['image_hits']} image)")
    ...
    try:
        ...
        if analysis['image_details']:
            ...
            for img in analysis['image_details'][:5]:
                print(f"    - {img['segment_id']}: {img.get('vlm_description', 'No description')[:80]}...")
        ...
        print(f"Image Extraction: {'✓ Working' if analysis['image_segments'] > 0 else '✗ No images found'}")
        print(f"VLM Descriptions: {'✓ Working' if analysis['segments_with_vlm_description'] > 0 else '✗ Not generated'}")
        print(f"Dify Parity: {comparison['overall_status']}")

        # Save results
        output_path = Path("/Users/misaya.yanghejazfs.com.au/hejaz_projects/ai_gateway/ai-gateway/tests/integration/multimodal_test_results.json")
