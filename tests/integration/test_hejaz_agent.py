...
class HejazAgentTester:
    ...
    async def test_invoke(self, message: str, use_session: bool = False) -> dict | None:
        """测试同步调用"""
        print(f"\n📤 同步调用测试: {message[:50]}...")
    ...
    async def test_stream(self, message: str, use_session: bool = False) -> str | None:
        """测试流式调用"""
        print(f"\n📡 流式调用测试: {message[:50]}...")
                        ...
                        try:
                            ...
                            if tool_call:
                                ...
                                if event_type == "tool_call_start":
                                    ...
                                    result = content_data
                                    print(
                                        f"  [{chunk_count}] ✅ 工具结果: {tool_name} -> {result[:50]}..."
                                    )
