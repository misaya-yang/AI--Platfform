...
class KBTester:
    ...
    async def test_retrieval(
        ...
        try:
            ...
            resp = await self.client.post(
                f"/api/v1/knowledge/{self.dataset_id}/hit_test",
                json=payload
            )
    ...
    async def test_retrieve_with_images(
        self, 
        query: str,
        top_k: int = 5
    ) -> bool:
        ...
        try:
            ...
            resp = await self.client.post(
                f"/api/v1/knowledge/{self.dataset_id}/retrieve",
                json=payload
            )
    ...
    async def list_documents(self) -> bool:
        ...
        try:
            resp = await self.client.get(
                f"/api/v1/knowledge/{self.dataset_id}/documents"
            )
...
async def main():
    ...
    parser.add_argument(
        "--test-file", "-f",
        help="File with test questions (one per line)"
    )
    parser.add_argument(
        "--interactive", "-i",
        action="store_true",
    ...
    parser.add_argument(
        "--multimodal", "-M",
        action="store_true",
        help="Test multimodal retrieval with images"
    )
    parser.add_argument(
        "--base-url", "-u",
