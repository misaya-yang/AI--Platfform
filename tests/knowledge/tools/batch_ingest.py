...
class BatchIngestor:
    ...
    async def check_dataset(self) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"   Create it first with: POST /knowledge/datasets")
                return False
            else:
    ...
    async def upload_file(self, file_path: Path) -> Optional[str]:
        ...
        try:
            print(f"  📤 Uploading {file_path.name} ({file_path.stat().st_size / 1024 / 1024:.2f} MB)...", end=" ")
            ...
            if resp.status_code == 200:
                ...
                print(f"❌ Dataset not found (404)")
                return None
            elif resp.status_code == 413:
                print(f"❌ File too large (413)")
                return None
            else:
    ...
    async def wait_for_document(self, document_id: str, max_wait: int = 600) -> bool:
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                print(f"     ⚠️ Timeout checking status (attempt {consecutive_errors}/{max_consecutive_errors})")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Batch ingest PDF documents into knowledge base"
    )
    parser.add_argument(
        "--dataset", "-d",
        required=True,
        help="Dataset ID (e.g., 'imam')"
    )
    parser.add_argument(
        "--files", "-f",
        nargs="+",
        help="File patterns to upload (e.g., '*.pdf' or specific paths)"
    )
    parser.add_argument(
        "--dir", "-D",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of concurrent uploads (default: 2)"
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
    ...
    if not args.api_key:
        print("❌ API Key is required. Set GATEWAY_API_KEY environment variable or use --api-key flag.")
        sys.exit(1)
