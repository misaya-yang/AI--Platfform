...
resp = requests.delete(
    f"{API_BASE}/api/v1/knowledge/{DATASET_ID}/documents/{doc_id_old}",
    headers=headers
)
if resp.status_code == 200:
...
for i in range(90):  # 3分钟 = 90 * 2秒
    ...
    if resp.status_code == 200:
        ...
        if progress != last_progress:
            ...
            print(f"[{elapsed:.0f}s] 状态: {status:12s} | 进度: {progress:6.2f}% | 速度: {speed:.1f}%/分钟")
