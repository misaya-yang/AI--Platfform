...
for i in range(150):  # 5分钟 = 150 * 2秒
    ...
    if resp.status_code == 200:
        ...
        if progress != last_progress:
            ...
            print(f"{elapsed:6.0f}s | {status:^12s} | {progress:7.2f}% | {speed:9.1f}%/分 | {remaining:7.1f}分")
...
if resp.status_code == 200:
    ...
    if segments:
        ...
        avg_tokens = sum(s.get("token_count", 0) for s in text_segs) / len(text_segs) if text_segs else 0
        print(f"   平均Token数: {avg_tokens:.0f}")

print("\n" + "=" * 80)
print("📈 性能对比:")
print(f"   优化前: ~25%/分钟 (预计需要4分钟)")
