...
if [ -e "$ENV_FILE" ] && [ "$FORCE" != true ]; then
    if [ "$IF_MISSING" = true ]; then
        ...
        upgraded=false
        for generated_key in GATEWAY_ENCRYPTION_KEY CODEX_RUNTIME_INTERNAL_TOKEN CODEX_RUNTIME_LEASE_SIGNING_SECRET CODEX_MODEL_PLANE_INTERNAL_TOKEN; do
...
CODEX_RUNTIME_INTERNAL_TOKEN_VALUE="$(copy_env_or_default CODEX_RUNTIME_INTERNAL_TOKEN "$(generate_secret)")"
CODEX_RUNTIME_LEASE_SIGNING_SECRET_VALUE="$(copy_env_or_default CODEX_RUNTIME_LEASE_SIGNING_SECRET "$(generate_secret)")"
CODEX_MODEL_PLANE_INTERNAL_TOKEN_VALUE="$(copy_env_or_default CODEX_MODEL_PLANE_INTERNAL_TOKEN "$(generate_secret)")"
...
value_for_key() {
    ...
    case "$key" in
        ...
        CODEX_RUNTIME_INTERNAL_TOKEN)
            printf '%s' "$CODEX_RUNTIME_INTERNAL_TOKEN_VALUE"
            ;;
        CODEX_RUNTIME_LEASE_SIGNING_SECRET)
            printf '%s' "$CODEX_RUNTIME_LEASE_SIGNING_SECRET_VALUE"
            ;;
        CODEX_MODEL_PLANE_INTERNAL_TOKEN)
            printf '%s' "$CODEX_MODEL_PLANE_INTERNAL_TOKEN_VALUE"
            ;;
