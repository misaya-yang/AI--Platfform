...
if [ -e "$ENV_FILE" ] && [ "$FORCE" != true ]; then
    if [ "$IF_MISSING" = true ]; then
        if ! awk -F= '$1 == "GATEWAY_ENCRYPTION_KEY" && length($0) > length($1) + 1 {found=1} END {exit !found}' "$ENV_FILE"; then
            upgrade_file="$(mktemp "${ENV_FILE}.upgrade.XXXXXX")"
            trap 'rm -f "$upgrade_file"' EXIT
            awk -F= '$1 != "GATEWAY_ENCRYPTION_KEY" {print}' "$ENV_FILE" > "$upgrade_file"
            printf 'GATEWAY_ENCRYPTION_KEY=%s\n' "${GATEWAY_ENCRYPTION_KEY:-$(generate_secret)}" >> "$upgrade_file"
