#!/usr/bin/env python3
"""Generate and validate the pinned Codex Harness supply-chain receipt."""
...
LOCK_SCHEMA = "ai-platform/codex-harness-lock/v2"
RECEIPT_SCHEMA = "ai-platform/codex-harness-source-receipt/v1"
ARTIFACT_BINARIES = {
...
def generate_receipt(*, fork: Path, schema_dir: Path, receipt_path: Path, sbom_path: Path) -> None:
    ...
    if _run(["git", "status", "--porcelain"], cwd=fork):
        raise ContractError("Codex Harness fork must be clean before generating a source receipt")
    ...
    serial = uuid.uuid5(
        uuid.NAMESPACE_URL,
        f"ai-platform-codex:{fork_sha}:{tree_sha}",
    )
    sbom = {
        "bomFormat": "CycloneDX",
        "specVersion": "1.5",
        ...
        "metadata": {
            "component": {
                ...
                "name": "ai-platform-codex-harness",
                "version": fork_sha,
                "properties": [
...
def validate_lock(
    *,
    ...
    lock = _load_object(lock_path, label="Codex Harness lock")
    ...
    receipt = _load_object(receipt_path, label="Codex Harness source receipt")
    ...
    if license_info.get("spdx") != "Apache-2.0":
        raise ContractError("Codex Harness license must remain Apache-2.0")
...
def record_local_image(
    *,
    ...
    expected_binary = ARTIFACT_BINARIES[artifact_id]
    expected_labels = {
        "org.opencontainers.image.revision": source["fork_sha"],
        "com.misaya.ai-platform.codex.schema-sha256": build["app_server_schema_sha256"],
        "com.misaya.ai-platform.codex.artifact": artifact_id,
        "com.misaya.ai-platform.codex.binary": expected_binary,
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"codex-harness-contract: {exc}", file=sys.stderr)
        return 1
    print("codex-harness-contract: ok")
    return 0
