#!/usr/bin/env python3
"""Real Qwen Responses gate for the isolated Codex candidate Runtime.
...
from src.api.internal.codex_model_plane import router as model_plane_router
from src.services.codex_runtime import (
    CodexModelPlane,
...
KERNEL_REVISION = "codex-runtime-text-gate"
    ...
    try:
        artifact = _docker(
            [
                ...
                image,
                "--format",
                '{{index .Config.Labels "com.misaya.ai-platform.codex.artifact"}}',
            ],
            capture=True,
        )
        binary = _docker(
            [
                ...
                image,
                "--format",
                '{{index .Config.Labels "com.misaya.ai-platform.codex.binary"}}',
            ],
            capture=True,
        )
    except GateError:
...
class GateConfig:
    ...
    def load(cls, *, env_file: Path, runtime_image: str) -> GateConfig:
        ...
        if not _runtime_image_is_valid(runtime_image):
            raise GateError("CODEX_RUNTIME_IMAGE is not a locked Agent Runtime artifact")
        try:
            ttft_limit = float(value("CODEX_RUNTIME_TEXT_TTFT_LIMIT_SECONDS") or "8")
            long_min_chars = int(value("CODEX_RUNTIME_LONG_OUTPUT_MIN_CHARS") or "700")
        except ValueError as exc:
...
class _ObservedModelPlane(CodexModelPlane):
...
class LiveTextGate:
    def __init__(self, config: GateConfig) -> None:
        ...
        self.config = config
        self.network = f"codex-text-gate-{suffix}"
        self.postgres = f"codex-text-gate-pg-{suffix}"
        self.runtime = f"codex-text-gate-runtime-{suffix}"
        ...
        self.control: CodexRuntimeControlPlane | None = None
    ...
    async def _start(self) -> None:
        ...
        self.control = CodexRuntimeControlPlane(
            ...
            model_plane_base_url=(
                f"http://host.docker.internal:{self.model_plane_port}/internal/v1/codex-model-plane"
            ),
            kernel_revision=KERNEL_REVISION,
        )
    ...
    async def _initialize_database(self) -> None:
        ...
        async with self.pool.acquire() as connection:
            ...
            await connection.execute(
                ...
                ) VALUES ($3, $2, $1, 'codex_candidate', $4, 'live_gate')
                """,
                self.session_id,
                self.user_id,
                self.tenant_id,
    ...
    async def _start_model_plane(self) -> None:
        ...
        app.state.codex_model_plane_internal_token = self.model_plane_token
        app.state.codex_model_plane = self.model_plane
    ...
    def _start_runtime_container(self) -> None:
        _docker(
            [
                ...
                "256",
                "--tmpfs",
                "/var/lib/ai-platform-codex/runtime-home:rw,uid=10001,gid=10001,mode=0700",
                "--env",
                ...
                "--env",
                f"CODEX_MODEL_PLANE_INTERNAL_TOKEN={self.model_plane_token}",
                "--env",
    ...
    async def run_turn(
        ...
        *,
        ...
        if (
            ...
            or run["engine"] != "codex_harness"
        ):
            raise GateError(f"{label} did not close one Codex-only billing path")
        return TurnReceipt(
            label=label,
            run_id=turn.run_id,
            text=text,
...
async def _run(config: GateConfig) -> None:
    ...
    async with LiveTextGate(config) as gate:
        ...
        print("CODEX_RUNTIME_TEXT_GATE_OK")
...
def main() -> int:
    ...
    parser.add_argument("--runtime-image", default=os.environ.get("CODEX_RUNTIME_IMAGE", ""))
    arguments = parser.parse_args()
    if not arguments.runtime_image:
        print("ERROR: CODEX_RUNTIME_IMAGE is required")
        return 2
    try:
