...
class Container:
    ...
    def _register_providers(self) -> None:
        ...
        # 任务存储
        self._providers["task_storage"] = Provider(
            self._create_task_storage, singleton=True
        )
        ...
        # 会话管理器
        self._providers["session_manager"] = Provider(
            self._create_session_manager, singleton=True
        )
        
        # 任务管理器
        self._providers["task_manager"] = Provider(
            self._create_task_manager, singleton=True
        )
        
        # 任务队列
        self._providers["task_queue"] = Provider(
            self._create_task_queue, singleton=True
        )
        ...
        # 健康监控
        self._providers["health_monitor"] = Provider(
            self._create_health_monitor, singleton=True
        )
        
        # 负载均衡器
        self._providers["load_balancer"] = Provider(
            self._create_load_balancer, singleton=True
        )
        
        # ========== LangGraph 相关 ==========
        
        # LangGraph 代理
        self._providers["langgraph_proxy"] = Provider(
            self._create_langgraph_proxy, singleton=True
        )
    ...
    def _create_langgraph_proxy(self):
        ...
        for inst_cfg in self.settings.langgraph.instances:
            instances.append(LangGraphInstance(
    ...
    def _create_user_resolver(self):
        ...
        return UserResolver(UserResolverConfig(
