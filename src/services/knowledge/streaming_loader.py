...
class StreamingDocumentLoader:
    ...
    def _sync_get_page_count(self, source: Union[bytes, str]) -> int:
        ...
        try:
            ...
            for start_idx in range(0, total_pages, self.batch_size):
                ...
                pages = await asyncio.to_thread(
                    self._extract_pages_batch, doc, start_idx, end_idx
                )
    ...
    async def load_page_range(
        ...
        return await asyncio.to_thread(
            self._sync_load_page_range, source, start_page, end_page
        )
    ...
    def _open_document(self, source: Union[bytes, str]) -> Any:
