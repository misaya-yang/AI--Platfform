...
class IslamicTextChunker(BaseChunker):
    ...
    def _chunk_quran_tanzil_lines(self, verses: list[tuple[int, int, str]]) -> list[Chunk]:
        ...
        max_limit = (
            self.config.token_limit if self.config.use_token_count else self.config.chunk_size
        )

        current_sura: int | None = None
        current_start_ayah: int | None = None
        current_end_ayah: int | None = None
        ...
        for sura, ayah, verse_text in verses:
            ...
            verse_size = (
                token_counter.count_tokens(verse_text) if token_counter else len(verse_text)
            )
