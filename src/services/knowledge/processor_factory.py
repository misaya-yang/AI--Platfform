...
class TextProcessor(DocumentProcessor):
    ...
    async def process_batch(
        ...
        # Apply chunking
        from .chunking import ChunkingConfig, create_chunker
...
class ScannedProcessor(DocumentProcessor):
    ...
    async def process_batch(
        ...
        try:
            ...
            for (page_num, img_bytes), vector in zip(page_images, vectors, strict=False):
                if vector:
                    chunks.append(
                        ProcessedChunk(
    ...
    async def finalize(
        ...
        if text_chunks:
            ...
            try:
                ...
                for chunk, vector in zip(text_chunks, vectors, strict=False):
                    chunk.vector = vector
            except Exception as e:
                logger.error(f"Text embedding failed: {e}")
...
class ProcessorFactory:
    ...
    def __init__(
        self,
        text_embedder: Any,
        multimodal_embedder: Any | None = None,
        ocr_service: Any | None = None,
        default_chunking_config: Any | None = None,
    ):
        """
