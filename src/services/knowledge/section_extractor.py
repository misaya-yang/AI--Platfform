...
class Section:
    ...
    end_pos: int | None = None  # Character position where section ends
...
class SectionExtractor:
    ...
    def __init__(
        ...
        if include_english:
            ...
            self.patterns.append(
                (re.compile(self.ALL_CAPS_PATTERN[0], re.MULTILINE), self.ALL_CAPS_PATTERN[1])
            )
    ...
    def extract_sections(self, text: str) -> list[Section]:
        ...
        for i, (start, _end, title, level) in enumerate(matches):
            ...
            section = Section(
                title=title, level=level, start_pos=start, end_pos=section_end, parent=parent
            )
    ...
    def get_section_for_position(self, sections: list[Section], pos: int) -> Section | None:
        """
    ...
    def assign_section_to_chunks(
        self, text: str, chunks: list[Any], chunk_positions: list[int] | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for _i, (_chunk, pos) in enumerate(zip(chunks, chunk_positions, strict=False)):
    ...
    def _find_nearest_section(self, sections: list[Section], pos: int) -> Section | None:
    ...
    def _clean_title(self, title: str) -> str:
        ...
        title = re.sub(
            r"^(?:باب|كتاب|فصل|مسألة|فرع|Chapter|Section|Book|Part)\s*[:：.]?\s*",
            "",
            title,
            flags=re.IGNORECASE,
        )
    ...
    def force_section_title(
        self,
        chunks: list[Any],
        document_title: str | None = None,
        default_section: str = "Main Content",
    ) -> list[dict[str, Any]]:
        """
        ...
        for chunk in chunks:
            if hasattr(chunk, "metadata") and chunk.metadata:
                existing_section = chunk.metadata.get("section_title") or chunk.metadata.get(
                    "heading"
                )
                if existing_section:
...
def extract_sections_with_traceability(
    text: str, chunks: list[Any], strict: bool = True, document_title: str | None = None
) -> list[dict[str, Any]]:
    """
...
def get_section_aware_citation(
    chunk_metadata: dict[str, Any], document_name: str | None = None, position: int | None = None
) -> str:
    """
