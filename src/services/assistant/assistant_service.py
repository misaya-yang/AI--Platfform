        ...
        while iteration < max_tool_iterations:
            ...
            for idx in sorted(tool_calls_accumulated.keys()):
                ...
                if tool_name == "execute_python_code" and self.code_executor:
                    ...
                    # Quiz generation tool — retrieve KB, generate via LLM, emit quiz:ready
                    try:
                        yield AssistantStreamEvent(
                        ...
                        quiz_topic = tool_args.get("topic", "key concepts")
                        quiz_count = min(10, max(1, int(tool_args.get("question_count", 5))))
                        quiz_types = tool_args.get("question_types", ["mc_single"])
                        quiz_difficulty = tool_args.get("difficulty", "medium")
                        quiz_language = tool_args.get("language", "auto")

                        # Retrieve KB chunks
                        quiz_chunks: list[dict] = []
                        kb_datasets = config.kb_dataset_ids or []
                        for ds_id in kb_datasets:
                            try:
                                results, _ = await self.kb_service.retrieve(
                                    user=user, dataset_id=ds_id, query=quiz_topic,
                                    top_k=20, mode="hybrid",
                                )
                                for r in results:
                                    quiz_chunks.append({"content": r.text, "score": r.score, "metadata": r.metadata or {}})
                            except Exception as qe:
                                logger.warning(f"Quiz KB retrieval failed for {ds_id}: {qe}")

                        if not quiz_chunks:
                            tool_results.append({
                                "tool_call_id": tool_id, "role": "tool",
                                "content": "Error: No content retrieved from knowledge base. Ensure datasets have indexed documents.",
                            })
                        else:
                            yield AssistantStreamEvent(
                                event_type="quiz:status",
                                data={"message": f"Retrieved {len(quiz_chunks)} passages. Generating questions..."},
                            )
                            ...
                            quiz_gen = QuizGenerator(self.model_registry)
                            quiz_grader = QuizGrader(model_registry=self.model_registry)
                            quiz_svc = QuizService(db=self.db, generator=quiz_gen, grader=quiz_grader)

                            quiz_result = await quiz_svc.create_quiz(
                                tenant_id=user.tenant_id, user_id=user.user_id,
                                dataset_ids=kb_datasets, kb_chunks=quiz_chunks,
                                topic=quiz_topic, question_count=quiz_count,
                                question_types=quiz_types, difficulty=quiz_difficulty,
                                language=quiz_language,
                            )

                            yield AssistantStreamEvent(event_type="quiz:status", data={"message": "Quiz ready!"})
                            yield AssistantStreamEvent(event_type="quiz:ready", data=quiz_result)

                            tool_results.append({
                                "tool_call_id": tool_id, "role": "tool",
                                "content": f"Quiz generated successfully: '{quiz_result['title']}' with {quiz_result['question_count']} questions. The quiz is now displayed as an interactive card in the chat.",
                            })

                    except Exception as e:
                        logger.error(f"Quiz generation tool failed: {e}", exc_info=True)
                        tool_results.append({
                            "tool_call_id": tool_id, "role": "tool",
                            "content": f"Quiz generation failed: {e}",
                        })
