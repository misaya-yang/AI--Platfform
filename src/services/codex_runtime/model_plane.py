"""Strictly private, lease-bound model-only data plane for Codex.
...
protocol consumed by the Codex kernel.
"""
...
class CodexModelPlaneError(RuntimeError):
...
def _timestamp_ms(value: datetime) -> int:
    if not isinstance(value, datetime) or value.tzinfo is None:
        raise CodexModelPlaneError("RUNTIME_MODEL_LEASE_TIME_INVALID", status_code=503)
...
def _chat_completions_url(base_url: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise CodexModelPlaneError("RUNTIME_PROVIDER_ENDPOINT_INVALID", status_code=503)
...
def _function_tool(
    ...
    *,
    ...
    if (
        ...
        raise CodexModelPlaneError("RUNTIME_TOOL_SCHEMA_INVALID", status_code=422)
    tool = {
        "type": "function",
...
def _validated_native_tools(value: Any, profile: Mapping[str, Any]) -> _ValidatedNativeTools:
    ...
    if not isinstance(tool_capabilities, Mapping):
        raise CodexModelPlaneError("RUNTIME_TOOL_CAPABILITY_INVALID", status_code=422)
    ...
    for raw in value:
        ...
        if tool_type == "web_search":
            if web_search_wire != "native":
                raise CodexModelPlaneError("RUNTIME_TOOL_SCHEMA_UNSUPPORTED", status_code=422)
...
class _ResponsesProjector:
    ...
    def complete(self) -> list[bytes]:
        ...
        if not self.text:
            raise CodexModelPlaneError("RUNTIME_PROVIDER_EMPTY_RESPONSE", status_code=502)
        usage = self.usage or {
...
class _NativeResponsesStreamValidator:
    ...
    def consume(self, payload: str) -> bytes | None:
        ...
        if not isinstance(event, dict):
            raise CodexModelPlaneError("RUNTIME_PROVIDER_STREAM_INVALID", status_code=502)
    ...
    def finish(self) -> _NativeResponsesTerminal:
        if not self.seen_created or self.terminal is None:
            raise CodexModelPlaneError("RUNTIME_PROVIDER_STREAM_INCOMPLETE", status_code=502)
        return self.terminal


class CodexModelPlane:
    def __init__(
        self,
        *,
        database: _Database,
    ...
    async def authorize_and_reserve(
        ...
        *,
        ...
        try:
            ...
            raise CodexModelPlaneError("RUNTIME_MODEL_LEASE_INVALID", status_code=401) from None
        ...
        if row is None:
            raise CodexModelPlaneError("RUNTIME_MODEL_LEASE_NOT_FOUND", status_code=401)
        data = dict(row)
        ...
        try:
            ...
            raise CodexModelPlaneError(exc.code, status_code=401) from None
        ...
        try:
            ...
            if "MODEL_CALL_REPLAYED" in code:
                raise CodexModelPlaneError("RUNTIME_MODEL_CALL_REPLAYED", status_code=409) from None
            if "BUDGET_EXHAUSTED" in code:
                raise CodexModelPlaneError(
                    "RUNTIME_MODEL_LEASE_BUDGET_EXHAUSTED", status_code=429
                ) from None
            raise CodexModelPlaneError("RUNTIME_MODEL_LEASE_REJECTED", status_code=403) from None
    ...
    async def stream(
        ...
        *,
        ...
        if _provider_revision(provider.get("updated_at")) != call.provider_revision:
            ...
            raise CodexModelPlaneError("RUNTIME_PROVIDER_REVISION_CHANGED", status_code=409)
        ...
        if not api_key or not base_url:
            ...
            raise CodexModelPlaneError("RUNTIME_PROVIDER_UNAVAILABLE", status_code=503)
        ...
        if wire_protocol != "chat_completions":
            ...
            raise CodexModelPlaneError("RUNTIME_PROVIDER_WIRE_UNSUPPORTED", status_code=422)
        ...
        try:
            ...
            raise CodexModelPlaneError("RUNTIME_REASONING_WIRE_INVALID", status_code=422) from None
...
__all__ = ["CodexModelPlane", "CodexModelPlaneError"]
