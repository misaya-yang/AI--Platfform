...
def _build_config(body: dict):
    ...
    mr = getattr(app.state, "model_registry", None)
    if mr:
        mi = mr.get_model(model_id)
        if mi:
...
async def chat(request: Request):
    ...
    history = [{"role": m["role"], "content": m["content"]} for m in body["history"]] if body.get("history") else None
...
async def chat_stream(request: Request):
    ...
    async def event_generator():
        try:
            ...
            yield f"data: {json.dumps({'event_type': 'error', 'data': {'message': 'Stream error occurred'}, 'timestamp': time.time()})}\n\n"
