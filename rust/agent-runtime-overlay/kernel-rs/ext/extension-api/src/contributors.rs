...
pub trait ToolLifecycleContributor: Send + Sync {
    /// Runs immediately before the concrete handler. Any rejection prevents
    /// the handler from being called and is returned to the model.
    fn before_tool_dispatch<'a>(
        &'a self,
        _input: &'a ToolStartInput<'a>,
    ) -> ToolDispatchFuture<'a> {
