...
#[derive(Default)]
struct RecordingExtensionInstaller {
    install_calls: AtomicUsize,
}

impl AppServerExtensionInstaller for RecordingExtensionInstaller {
    fn install(&self, _builder: &mut ExtensionRegistryBuilder<Config>) {
        self.install_calls.fetch_add(1, Ordering::Relaxed);
    }
}

#[test]
fn in_process_host_runtime_uses_injected_store_and_extensions() -> Result<()> {
    run_with_codex_stack("in-process-host-runtime", async {
        ...
        let installer = Arc::new(RecordingExtensionInstaller::default());
        let host_runtime = AppServerHostRuntime::new()
            .with_thread_store(thread_store.clone())
            .with_extension_installer(installer.clone());
        let client =
        ...
        assert_eq!(thread.id, reserved_thread_id.to_string());
        assert_eq!(thread_store.calls().await.create_thread, 1);
        assert_eq!(installer.install_calls.load(Ordering::Relaxed), 1);
        ...
        let invalid_request = client
            .request_thread_start(
                ...
                AppServerThreadStartOptions::new(ThreadId::new()),
            )
            .await
            .expect_err("host options must be limited to thread/start");
        assert_eq!(invalid_request.kind(), std::io::ErrorKind::InvalidInput);

        let ephemeral_error = client
            .request_thread_start(
                ClientRequest::ThreadStart {
            ...
            .await?
            .expect_err("ephemeral threads cannot claim a durable host identity");
        assert_eq!(
            ephemeral_error.message,
...
fn run_with_codex_stack<F>(name: &str, future: F) -> Result<()>
where
    F: Future<Output = Result<()>> + Send + 'static,
{
    const CODEX_TEST_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;
    let handle = std::thread::Builder::new()
        .name(name.to_string())
        .stack_size(CODEX_TEST_STACK_SIZE_BYTES)
        .spawn(move || -> Result<()> {
...
async fn start_in_process_client_with_host(
    config: Arc<Config>,
    loader_overrides: LoaderOverrides,
    host_runtime: AppServerHostRuntime,
) -> std::io::Result<InProcessClientHandle> {
    in_process::start_with_host(
        InProcessStartArgs {
            arg0_paths: Arg0DispatchPaths::default(),
            config,
            cli_overrides: Vec::new(),
