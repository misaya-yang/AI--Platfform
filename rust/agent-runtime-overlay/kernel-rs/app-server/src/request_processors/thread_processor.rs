...
async fn stage_pending_project_metadata(
    ...
    stage_pending_project_metadata_for_thread(thread_store, thread_id, project_id, operation)
        .await?;
    Ok(Some(thread_id))
}
...
impl ThreadRequestProcessor {
    ...
    async fn thread_start_task(
        ...
        if start_options.config.ephemeral && thread_start_options.is_some() {
            return Err(invalid_request(
                "host-reserved thread identity cannot be used for an ephemeral thread",
            ));
        }
        let (reserved_thread_id, pending_project_thread_id) = if start_options.config.ephemeral {
            (None, None)
        } else if let Some(options) = thread_start_options {
            let thread_id = options.reserved_thread_id();
            ...
            (Some(thread_id), project_id.as_ref().map(|_| thread_id))
        } else {
            let thread_id = stage_pending_project_metadata(
        ...
        let NewThread {
            ...
            Err(err) => {
                remove_pending_project_metadata(thread_store.as_ref(), pending_project_thread_id)
                    .await;
