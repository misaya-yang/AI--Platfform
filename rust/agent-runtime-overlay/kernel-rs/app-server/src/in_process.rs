...
impl InProcessClientSender {
    ...
    /// Sends one `thread/start` request with a host-reserved durable identity.
    ...
    pub async fn request_turn_start(
        ...
        if !matches!(&request, ClientRequest::TurnStart { .. }) {
            return Err(IoError::new(
                ErrorKind::InvalidInput,
...
impl InProcessClientHandle {
    ...
    pub async fn request_thread_start(
        ...
        self.client.request_thread_start(request, options).await
    }
...
pub async fn start(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    start_with_host(args, AppServerHostRuntime::default()).await
}

/// Starts an in-process app-server with process-scoped host dependencies.
///
/// This is the supported embedding seam for durable thread stores and typed
/// platform extensions. The app-server remains the only owner of Thread/Turn/
/// Item lifecycle, tool-loop, compaction, and interrupt semantics.
pub async fn start_with_host(
    ...
    let client = start_uninitialized(args, host_runtime).await?;
                ...
                message = client_rx.recv() => {
                    match message {
                        Some(InProcessClientMessage::Request {
                            ...
                            match processor_tx.try_send(ProcessorCommand::Request {
                                request: Box::new(request),
                                thread_start_options,
                                turn_start_options,
                            }) {
