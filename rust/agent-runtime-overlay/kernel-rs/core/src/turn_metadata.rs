...
const PLATFORM_LEASE_KEYS: [&str; 3] = [
    "ai_platform_lease_id",
    "ai_platform_lease_signature",
    "ai_platform_scope_sha256",
];

/// Return only the complete platform lease tuple. Arbitrary client metadata,
/// reserved core fields, and partial tuples never cross into child config.
pub(crate) fn platform_lease_metadata_from(
    metadata: &BTreeMap<String, String>,
) -> BTreeMap<String, String> {
    if !PLATFORM_LEASE_KEYS
        .iter()
...
pub(crate) fn platform_lease_metadata_for_child(
    session_source: &SessionSource,
    metadata: &BTreeMap<String, String>,
) -> BTreeMap<String, String> {
    if !matches!(
    ...
    platform_lease_metadata_from(metadata)
}

fn merged_platform_lease_metadata(
    client_metadata: &BTreeMap<String, String>,
    inherited_metadata: &BTreeMap<String, String>,
) -> BTreeMap<String, String> {
    let client = platform_lease_metadata_from(client_metadata);
    let inherited = platform_lease_metadata_from(inherited_metadata);
    match (client.is_empty(), inherited.is_empty()) {
        (false, false) if client != inherited => BTreeMap::new(),
        (false, _) => client,
        (_, false) => inherited,
        _ => BTreeMap::new(),
    }
...
impl TurnMetadataState {
    ...
    pub(crate) fn platform_lease_metadata_for_child(
        ...
        let client_metadata = self
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone();
        let inherited_metadata = self
            .responses_api_metadata
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone();
        let metadata = merged_platform_lease_metadata(&client_metadata, &inherited_metadata);
        platform_lease_metadata_for_child(session_source, &metadata)
    }
...
#[cfg(test)]
mod platform_lease_metadata_tests {
    use super::merged_platform_lease_metadata;
    use super::platform_lease_metadata_for_child;
    use super::platform_lease_metadata_from;
    use codex_protocol::protocol::SessionSource;
    use std::collections::BTreeMap;

    #[test]
    fn only_complete_platform_lease_tuple_is_inherited() {
        let metadata = BTreeMap::from([
            ("ai_platform_lease_id".to_string(), "lease".to_string()),
            (
                "ai_platform_lease_signature".to_string(),
                "signature".to_string(),
            ),
            ("ai_platform_scope_sha256".to_string(), "scope".to_string()),
            ("unrelated".to_string(), "must_not_copy".to_string()),
        ]);
        let inherited = platform_lease_metadata_from(&metadata);
        assert_eq!(inherited.len(), 3);
        assert_eq!(inherited["ai_platform_lease_id"], "lease");
        assert!(!inherited.contains_key("unrelated"));
        assert!(!inherited.contains_key("thread_id"));
        assert!(!inherited.contains_key("turn_id"));
        assert!(!inherited.contains_key("root_turn_id"));
        assert!(!inherited.contains_key("parent_thread_id"));
        assert!(
            platform_lease_metadata_from(&BTreeMap::from([(
    ...
    #[test]
    fn non_subagent_sources_never_inherit_platform_lease() {
        let metadata = BTreeMap::from([
            ("ai_platform_lease_id".to_string(), "lease".to_string()),
            (
                "ai_platform_lease_signature".to_string(),
        ...
        assert!(platform_lease_metadata_for_child(&SessionSource::Cli, &metadata).is_empty());
    }

    #[test]
    fn inherited_child_tuple_can_flow_to_a_grandchild_but_conflicts_fail_closed() {
        ...
        assert_eq!(
            merged_platform_lease_metadata(&BTreeMap::new(), &tuple),
            tuple
        );
        ...
        assert!(merged_platform_lease_metadata(&tuple, &conflict).is_empty());
    }
}
