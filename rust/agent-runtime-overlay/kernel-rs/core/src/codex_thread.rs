...
impl CodexThread {
    ...
    /// Starts an idle thread using a host-reserved turn id.
    ///
    /// This is for in-process platform hosts that must persist authorization
    /// before model dispatch. Public App Server callers cannot select ids.
    #[doc(hidden)]
    pub async fn start_turn_with_id(
        ...
        self.submit_turn_input_with_mode_and_id(
            request,
    ...
    async fn submit_turn_input_with_mode_and_id(
        &self,
        request: TurnInputRequest,
        mode: TurnInputMode,
        reserved_turn_id: String,
        ...
        self.io
            .submit_turn_input_with_id(request, mode, reserved_turn_id)
            .await
    }
