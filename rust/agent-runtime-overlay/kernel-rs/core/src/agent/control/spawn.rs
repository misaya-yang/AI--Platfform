...
impl AgentControl {
    ...
    async fn spawn_agent_internal(
        ...
        // A child model call must spend the same signed root lease budget as
        // its parent, but only the three platform lease fields are inherited.
        // Core continues to generate child thread/turn/root/parent metadata.
        if matches!(
