...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any_with_terminal_outcome(
        ...
        if tool.mcp_server_name().is_none() {
            let effect = effective_tool_effect(tool.as_ref(), &invocation);
            if let Err(gate) = notify_tool_start(&invocation, /*mcp_tool*/ None, effect).await {
                let message = if gate.message.is_empty() {
                    "tool dispatch blocked by platform policy".to_string()
                } else {
                    gate.message
                };
                let err = FunctionCallError::RespondToModel(message);
...
fn effective_tool_effect(
    tool: &dyn CoreToolRuntime,
    invocation: &ToolInvocation,
) -> codex_tools::ToolEffect {
    if let Some(command) = shell_script_for_invocation(invocation) {
