...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        Self::start_with_host(args, AppServerHostRuntime::default()).await
    }

    /// Starts the in-process runtime with host-owned persistence and extensions.
    pub async fn start_with_host(
        args: InProcessClientStartArgs,
        host_runtime: AppServerHostRuntime,
    ) -> IoResult<Self> {
        let channel_capacity = args.channel_capacity.max(1);
        let mut handle = codex_app_server::in_process::start_with_host(
    ...
    /// Starts a thread using a host-reserved durable identity.
...
impl InProcessAppServerRequestHandle {
    ...
    /// Resolve a server-originated request from a bounded runtime event
    /// handler without requiring ownership of the foreground client.
    pub fn resolve_server_request(
        ...
        self.command_tx
            .try_send(ClientCommand::ResolveServerRequest {
                request_id,
                result,
                response_tx,
            })
            .map_err(|_| IoError::new(ErrorKind::WouldBlock, "server request queue is full"))
    }

    pub async fn resolve_server_request_async(
        &self,
        request_id: RequestId,
        result: JsonRpcResult,
    ) -> IoResult<()> {
        ...
        self.command_tx
            ...
            .await
            .map_err(|_| IoError::new(ErrorKind::BrokenPipe, "server request queue is closed"))?;
        response_rx
            .await
            .map_err(|_| IoError::new(ErrorKind::BrokenPipe, "server request response is closed"))?
    }
    ...
    pub fn reject_server_request(
        ...
        let (response_tx, _response_rx) = oneshot::channel();
        self.command_tx
            .try_send(ClientCommand::RejectServerRequest {
                request_id,
                error,
                response_tx,
            })
    ...
    pub async fn reject_server_request_async(
        &self,
        request_id: RequestId,
        error: JSONRPCErrorError,
    ) -> IoResult<()> {
