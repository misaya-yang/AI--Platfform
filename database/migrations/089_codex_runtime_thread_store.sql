-- 089 — Codex Harness runtime ownership, immutable snapshots, and append-only items.
--
-- Additive only. Existing sessions and Python AgentLoop runs remain unchanged.
-- The new tables are the durable Thread/Turn/Item authority for Codex-owned
...
CREATE TABLE IF NOT EXISTS assistant_session_runtime_assignments (
    ...
    CONSTRAINT assistant_session_runtime_assignments_owner_check
        CHECK (runtime_owner IN ('python_control', 'codex_candidate')),
    CONSTRAINT assistant_session_runtime_assignments_kernel_check
        CHECK (
            (runtime_owner = 'python_control' AND kernel_revision IS NULL)
            OR
            (runtime_owner = 'codex_candidate' AND kernel_revision IS NOT NULL)
        ),
...
CREATE TABLE IF NOT EXISTS assistant_runtime_threads (
    ...
    user_id VARCHAR(255) NOT NULL,
    session_id VARCHAR(255) NOT NULL,
    kernel_owner VARCHAR(32) NOT NULL DEFAULT 'codex',
    ...
    CONSTRAINT assistant_runtime_threads_kernel_check
        CHECK (kernel_owner IN ('python', 'codex')),
...
COMMENT ON TABLE assistant_runtime_threads IS
    'One durable Codex Thread owner per tenant/user/session; legacy import metadata is bounded and hash-addressed.';
COMMENT ON TABLE assistant_session_runtime_assignments IS
    'Immutable per-session selection of the Python control or Codex candidate runtime; assignment never depends on prompt content.';
COMMENT ON TABLE assistant_runtime_thread_members IS
    'Immutable root, subagent, and fork membership for every Codex Thread in one platform session.';
COMMENT ON TABLE assistant_runtime_thread_projections IS
    'Mutable discovery projection for immutable Codex member identity and append-only history.';
...
COMMENT ON TABLE assistant_runtime_items IS
    'Append-only Codex Thread/Turn/Item event log with per-thread monotonic sequence and idempotent event keys.';
